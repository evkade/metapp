  ...
  async (req: Request, res: Response, next: NextFunction) => {
    if (req.query && req.query.currentbar) {
      ...
      if (currentbar === "dkm" || currentbar === "mkm") {
        const body = await getBeers(currentbar).then(data => data).catch(err => { next(new ErrorException(ErrorCode.badRequest, err)) });
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    if (req.query && req.query.currentbar) {
      ...
      if (currentbar === "dkm" || currentbar === "mkm") {
        const currentbar = (req.query as any).currentbar;
        const body = await getActiveBeers(currentbar).then(data => data).catch(err => { next(new ErrorException(ErrorCode.badRequest, err)) });
        if (body) res.status(200).send(body);
        else next(new ErrorException(ErrorCode.BeverageNotFound))
      }
    }
