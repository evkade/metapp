...
router.get("/api/cocktail", async (req: Request, res: Response) => {
  if (req.query && req.query.currentpub) {
    ...
    const body = await getCocktails(currentpub)
      .then((data) => data)
      .catch((err) => {
        res.status(404);
      });
...
router.get("/api/activecocktails", async (req: Request, res: Response) => {
  if (req.query && req.query.currentpub) {
    const currentpub = (req.query as any).currentpub;
    const body = await getActiveCocktails(currentpub)
      .then((data) => data)
      .catch((err) => {
        res.status(404);
      });
  ...
  async (req: Request, res: Response) => {
    ...
    if (req.query && req.query.currentpub && req.currentUser.isAdmin) {
      ...
      if (currentpub === "DKM" || currentpub === "MKM") {
        const body = await upsertCocktail(currentpub, req.body).then(
          (data) => data
        );
...
router.get("/api/cocktail/:id", async (req: Request, res: Response) => {
  if (req.query && req.query.currentpub && req.params && req.params.id) {
    ...
    if (!(currentpub === "DKM" || currentpub === "MKM")) {
      ...
      const body = await getCocktailById(currentpub, cocktailId).then(
        (data) => data
      );
      if (body) res.status(200).send(body);
      else res.status(404);
  ...
  async (req: Request, res: Response) => {
    ...
    if (
      ...
      if (!(currentpub === "DKM" || currentpub === "MKM")) {
        ...
        const body = await deleteCocktailById(currentpub, cocktailId).then(
          (data) => data
        );
        if (body) res.status(200).send(body);
        else res.status(404);
