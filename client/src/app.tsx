...
const App = () => {
  ...
  checkValidUser()
    .then(() => setUser(store.getState().user.username))
  ...
  return (
    <Provider store={store}>
      <Router>
        {(user || store.getState().user.loggedIn) && !store.getState().user.isAdmin 
          ? <MainNavbar signout={signOut} /> : null}
        <Switch>
          <AdminRoute 
            exact path="/modifyMenu" 
            drinkModel={drinkModel}
            searchType={searchTypes.COCKTAIL}
            component={AddBeverageToMenuPresenter} />
          <PrivateRoute 
            exakt path="/vieworders"
